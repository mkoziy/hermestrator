...
- **A `command/shell` `execute` step that exits non-zero writes no queryable data record at all, unless `ignoreExitCode: true` is set.** Without that flag, a failing step produces neither a `result` nor a `log` data output — confirmed empirically via `swamp model method run` + `swamp data query` returning zero results for a deliberately failing step. Any downstream step that reads that step's output via `data.query(...)`/`data.latest(...)` (e.g. a guard, or an always-run `report` job reading the preceding job's stdout) silently sees nothing on the failure path, even though the step "ran." The fix is `ignoreExitCode: true` on the step plus a separate `assert` step immediately after it (checking `attributes.exitCode == 0`) to restore the failure signal that `ignoreExitCode` otherwise swallows. See `workflows/workflow-github-ticket-actions.yaml`'s `run-actions` + `check-actions-exit` steps for the pattern. This is a general swamp-workflow footgun, not specific to that flow — any workflow with a "run job, then always-run report job that reads the run job's output" shape will hit it.
