...
- **Per-repo GitHub tokens are plain `GH_TOKEN_<OWNER>` env vars, not a swamp vault.** Fine-grained PATs are scoped to a single owner, so a repo under a different owner than the pod-level `GH_TOKEN` needs its own token. See `AGENTS.md`'s "GitHub tokens" section for the onboarding steps. A `local_encryption` vault was tried first and reverted: its auto-generated decryption key lives under `.swamp/` (gitignored, host-local), so it doesn't travel to wherever the orchestrator actually runs (e.g. a k3s pod) unless manually provisioned — plain env vars, injected the same way as `GH_TOKEN` already is, avoid that key-distribution problem.
